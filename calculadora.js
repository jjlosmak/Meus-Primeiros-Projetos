...
// Variáves para armazenarmos o valor, o operador e o estado da calculadora
...
const atualizaVisor = () => {
    ...
    if (parteInteira.length>10) {
        document.querySelector('#display').innerText = 'Numero Invalido!';
        return;
    }
    let v = '';
    c = 0;
...
// Tratamento no clique no botão de dígito
const digito = (n) => {
    if (ehNovoNumero) {
        sValor = '' + n;
        ehNovoNumero = false;
...
// Tratamento no clique no botão de ponto decimal
const virgula = () => {
    if (ehNovoNumero) {
        sValor = '0,';
        ehNovoNumero = false;
...
// Tratamento do clique do botão AC (All Clear)
const limpa = () => {
    ehNovoNumero = true;
    valorAnterior = 0;
    sValor = '0';
